...
#include "UnityCG.cginc"
	...
	fixed4 frag(vertexOut vo) : COLOR
	{
		...
		fixed4 bumpTex = tex2D(_BumpTex, vo.nuv); // range is from 0 to 1 to make the range to -1 to 1  = 2*value -1

		//unity store normals in alpha and green channel;
		float3 localCoordinates = float3(2 * bumpTex.ag - float2(1.0, 1.0), 0);
		localCoordinates.z = 1 - 0.5 * dot(localCoordinates, localCoordinates);
		...
		float3 normalDirection = normalize(mul(localCoordinates , tbnMatrix));
