Shader "3D/LambertSpecularShading"
{
	...
	vertexOut vert(vertexIn vi)
	{
		...
		float3	specular = pow(max(0.0,dot(reflect(-lightDirection, normalDirection) , viewDirection)) , _Shininess) * diffuse * _SpecularColor;

		float3 finalColor = diffuseFinal + UNITY_LIGHTMODEL_AMBIENT.xyz + specular;
		vo.uv = vi.uv;
		vo.color = float4(finalColor *_LightColor0 * _Color,0);
		return vo;
	}
