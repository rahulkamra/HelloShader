Shader "3D/LambertSpecularShading"
{
	...
	vertexOut vert(vertexIn vi)
	{
		...
		float 3diffuse = diffuse * _Power  + UNITY_LIGHTMODEL_AMBIENT.xyz;
		float3	specular = max(0.0,dot(reflect(-lightDirection, normalDirection) , viewDirection)) * diffuse;

		vo.uv = vi.uv;
		vo.color = float4(specular,0);
		float3 finalColor = diffuse * * _LightColor0.xyz * _Color
		return vo;
	}
