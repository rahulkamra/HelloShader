Shader "3D/LambertSpecularShading"
{
	...
	vertexOut vert(vertexIn vi)
	{
		...
		float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - mul(_Object2World , vi.position).xyz);
		...
		diffuse = diffuse * _Power;
		float3	specular = pow(max(0.0,dot(reflect(-lightDirection, normalDirection) , viewDirection)) * diffuse , _Shininess);

		vo.uv = vi.uv;
		vo.color = float4(specular,0);
		float3 finalColor = diffuse * _LightColor0.xyz * _Color + UNITY_LIGHTMODEL_AMBIENT.xyz;
		return vo;
	}
