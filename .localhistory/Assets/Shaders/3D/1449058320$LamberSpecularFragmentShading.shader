...
#pragma fragment frag
	...
	vertexOut vert(vertexIn vi)
	{
		...
		vo.posWorld = mul(_Object2World, vi.position).xyz;
		vo.inpPosition = vi.position;
		vo.uv = vi.uv;
		
		vo viewDirection = normalize(_WorldSpaceCameraPos.xyz - vo.posWorld);
	...
	fixed4 frag(vertexOut vo) : COLOR
	{
		...
		float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - vo.posWorld);
